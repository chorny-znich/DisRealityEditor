...
void ScreenManager::createScreen(GameData::Screens screenID)
{
	...
	else if (screenID == GameData::Screens::EDITOR) {
		std::unique_ptr<EditorScreen> pEditorScreen = std::make_unique<EditorScreen>();
		pEditorScreen->init();
		dr::GameState::addScreen(std::move(pEditorScreen));
	}
}